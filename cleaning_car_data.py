...
"""
i will first attempt to clean the data before handling the rest of the missing values.  my goal is to find a solution where i can impute missing values across the whole dataframe is accurate as possible.  but the first step is cleaning the data
into a form that i can work with.  this data came from an online car database with little to no restrictions on how people input values so the data is really messy.    
"""
...
cars_df['Year of putting into production'] = pd.to_numeric(cars_df['Year of putting into production'].fillna(' ').apply(lambda x: re.sub('year', '', x).strip()).replace(' ', np.nan), errors='coerce')
...
#investigate the Power column.  the Power columns needs to be cleaned.  i am going to split the Power column into a Power HP and Power RPM.
cars_df['Power']
...
cars_df['Power hp'], cars_df['Power rpm'] = cars_df['Power'].str.split('/', expand=True)[0], cars_df['Power'].str.split('/', expand=True)[1]
...
cars_df['Torque nm'], cars_df['Torque rpm'] = cars_df['Torque'].str.split('/', expand=True)[0], cars_df['Torque'].str.split('/', expand=True)[1]
...
cars_df['Engine displacement'] = pd.to_numeric(cars_df['Engine displacement'].apply(lambda x: get_replace(str(x), {' ':'', 'cm3':''})), errors='coerce')
...
cars_df['Year of stopping production'] = pd.to_numeric(cars_df['Year of stopping production'].fillna(' ').apply(lambda x: re.sub('year', '', x).strip()).replace(' ', np.nan), errors='coerce')
cars_df['Year of stopping production'].value_counts(dropna=False)
...
##################################################################################################################################################################
#  Missing data are the unobserved values that would be meaningful for analysis if observed; in other words, a missing value hides a meaningful value            #
##################################################################################################################################################################
